...
export default function Home() {
  ...
  return (
    ...
    >
      {/* Top bar */}
      <header className="flex items-center justify-between px-12 pt-7 pb-0">
        <span
        ...
        <div className="flex items-center gap-5 text-sm text-neutral-400">
          ...
          <a href="#" className="hover:text-neutral-800 transition-colors">IG</a>
          <a href="#" className="hover:text-neutral-800 transition-colors">TT</a>
          <a href="#" className="hover:text-neutral-800 transition-colors">Privacy</a>
        </div>
      </header>

      {/* Main content */}
      <main className="flex flex-1 items-center gap-12 px-12 py-10 max-w-[1200px] mx-auto w-full">

        {/* Left: Hero */}
        <div className="w-[360px] flex-shrink-0">
          <h1
            className="text-[58px] font-black leading-[1.05] tracking-[-0.02em] text-neutral-900 mb-5"
            style={{ fontFamily: "var(--font-display)" }}
          >
            Your c<span className="text-blue-500">u</span>rs<span className="text-blue-500">u</span>r<br />
            has <em>feelings.</em>
          </h1>
          <p className="text-[15px] text-neutral-400 font-light leading-relaxed mb-8 max-w-[280px]">
            Sunglasses while you watch. Sharp focus while you work. 70+ emotions, baked into your OS.
          </p>

          {/* Platform toggle */}
          <div className="flex items-center gap-1 rounded-full border border-neutral-200 p-1 w-fit mb-4">
            <button
        ...
        <div className="flex-1">
          {isMac ? (
            <div className="rounded-[18px] bg-gradient-to-b from-neutral-300 to-neutral-200 p-[10px] shadow-2xl shadow-neutral-300/50">
              <div className="rounded-[10px] overflow-hidden border border-neutral-300/60 bg-white">
                <div className="flex items-center gap-2 bg-neutral-100/80 border-b border-neutral-200 px-4 py-2.5">
                  <span className="w-3 h-3 rounded-full bg-red-400" />
                  <span className="w-3 h-3 rounded-full bg-yellow-400" />
                  <span className="w-3 h-3 rounded-full bg-green-400" />
                  <span className="flex-1 text-center text-[11px] text-neutral-400">cursur — live demo</span>
                </div>
                <DemoArea
                  demoRef={demoRef}
                  onMouseMove={handleMouseMove}
                  onEnter={() => setInsideDemo(true)}
                  onLeave={() => { setInsideDemo(false); setActiveZone(null); }}
                  zones={zones}
                  activeZone={activeZone}
                  insideDemo={insideDemo}
                  cursorPos={cursorPos}
            ...
            <div className="rounded-[6px] bg-slate-700 p-[10px] shadow-2xl shadow-slate-400/30">
              <div className="rounded-[2px] overflow-hidden bg-white">
                <div className="flex items-center bg-slate-800 px-3 py-2">
                  ...
                  <span className="text-[11px] text-slate-300">cursur — live demo</span>
...
function DemoArea({ demoRef, onMouseMove, onEnter, onLeave, zones, activeZone, insideDemo, cursorPos, activeLabel, windows }: {
  demoRef: React.RefObject<HTMLDivElement | null>;
  onMouseMove: (e: React.MouseEvent<HTMLDivElement>) => void;
  onEnter: () => void;
  onLeave: () => void;
  zones: typeof import("./page").default extends never ? never : { id: string; label: string; top: string; left: string; width: string; height: string }[];
  ...
  const radius = windows ? "2px" : "10px";
  const activeBorder = windows ? "border-blue-400 bg-blue-50" : "border-blue-200 bg-blue-50/70";
  const idleBorder = windows ? "border-slate-200 bg-white" : "border-neutral-200 bg-white";
  const deskBg = windows ? "bg-slate-100" : "bg-neutral-50";
  ...
  return (
    <div
      ...
      className={`relative ${deskBg} select-none overflow-hidden`}
      style={{ height: 360 }}
    >
      {zones.map((z) => (
        <div
          key={z.id}
          className={`absolute border transition-all duration-100 ${activeZone === z.id ? activeBorder : idleBorder}`}
          style={{ top: z.top, left: z.left, width: z.width, height: z.height, borderRadius: radius }}
        >
          <div className="p-3">
            {windows ? (
              <div className="flex items-center justify-between mb-2">
                <div className="h-2 w-12 bg-slate-100 rounded-sm" />
            ...
            <div className="space-y-1.5">
              <div className={`h-1.5 rounded-full w-3/4 ${windows ? "bg-slate-100" : "bg-neutral-100"}`} />
              <div className={`h-1.5 rounded-full w-1/2 ${windows ? "bg-slate-100" : "bg-neutral-100"}`} />
            </div>
          </div>
        </div>
      ))}
      ...
      {windows && (
        <div className="absolute bottom-0 left-0 right-0 h-7 bg-slate-800 flex items-center px-3 gap-2">
          <div className="w-3.5 h-3.5 bg-blue-500 rounded-sm flex items-center justify-center">
            <svg viewBox="0 0 88 88" className="w-2 h-2"><path d="M0 12.4l35.7-4.9v34.4H0zm39.9-5.5L87.3 0v41.5H39.9zM0 45.9h35.7v34.4L0 75.5zm39.9.4h47.4v41.3l-47.4-6.6z" fill="white"/></svg>
          </div>
      ...
      {!insideDemo && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <p className="text-xs text-neutral-300">Move your cursor here to try</p>
        </div>
      )}
    </div>
  );
